...
def save_optical_flow_png(filename:str, optical_flow: np.ndarray)-> None:
    ...
    optical_flow[..., 0] =  scale_factor * (optical_flow[..., 0] / width + 1.0)
    optical_flow[..., 1] =  scale_factor * (optical_flow[..., 1] / height + 1.0)
...
def load_optical_flow_png(filename: str)-> np.ndarray:
    ...
    optical_flow = np.asarray(cv2.imread(filename), dtype=np.float64)[..., ::-1]
